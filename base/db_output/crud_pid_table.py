...
from lingzhihua_db_models.models_all_table import PidTable   # 导入 PidTable 模型，注意文件位置
...
# 创建 pid_table 数据
def create_pid_table(db: Session, pid:int):
    db_pid_table = PidTable(pid=pid)
    db.add(db_pid_table)
    db.commit()
...
# 更新 pid_table 数据
def update_pid_table(db: Session, pid: int, ):
