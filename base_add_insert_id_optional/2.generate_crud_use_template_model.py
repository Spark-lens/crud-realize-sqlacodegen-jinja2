...
def generate_crud(database_url,output_dir="./db_output/"):
    ...
    for table_name in inspector.get_table_names():
        ...
        params = ", ".join("{0}:{1}".format(c['name'], TYPE_MAPPING.get(c['type'].__class__.__name__, 'Any')) for c in columns)
        non_primary_params = ", ".join("{0}:{1}".format(c['name'], TYPE_MAPPING.get(c['type'].__class__.__name__, 'Any')) 
        ...
        update_params = ", ".join("{0}: Optional[{1}] = None".format(c['name'], TYPE_MAPPING.get(c['type'].__class__.__name__, 'Any'))
                                    for c in columns if c['name'] != pk_name)
        ...
        output = template.render(
            ...
            params = params,
            non_primary_params = non_primary_params,
...
if __name__ == "__main__":
    ...
    # 示例调用
    # 推荐使用 psycopg2 驱动
    # python generate_crud_use_template_model.py postgresql+psycopg2://postgreRoot:hatech1618@localhost:5432/test
    # python generate_crud_use_template_model.py postgresql://postgreRoot:hatech1618@localhost/test